...
//								.padding([.bottom, .trailing])
					...
					if audioRecorder.recording == false {
						Button(action: {self.audioRecorder.startRecording(); recordCount+=1}) {
					...
					VStack {
						...
						case 2:
							SleepRecordSurveyView(recordMessage: "Recording isn't started. Press the red button to start.", survey: "1", audioRecorder: AudioRecorder(), showPopover: true)
						default:
...
//			.padding([.top, .trailing])
			...
			Form {
				Text("How well did you sleep?")
					...
					.padding([.top, .bottom])
				Picker("1: poor to 10: excellent", selection: $survey) {
				...
				Button(action: {
					self.audioRecorder.saveToCoreData(survey: survey)
				}) {
...
//				.alert("You selected \(survey)", isPresented: $showPopover) {
